			    ...
			    int flags)
{
  ...
  if (AmConfig::HideUserAgent) {
    // RFC 3261 §20.35: Server is optional; suppress to reduce fingerprinting surface
    removeHeader(reply.hdrs, SIP_HDR_SERVER);
  } else if (AmConfig::Signature.length() &&
             getHeader(reply.hdrs, SIP_HDR_SERVER).empty()) {
				  ...
				  msg_logger* logger)
{
  ...
  if (!AmConfig::HideUserAgent && AmConfig::Signature.length())
    reply.hdrs += SIP_HDR_COLSP(SIP_HDR_SERVER) + AmConfig::Signature + CRLF;
				  ...
				  int max_forwards)
{
  ...
  if (AmConfig::HideUserAgent) {
    // RFC 3261 §20.41: User-Agent is optional; suppress to reduce fingerprinting surface
    removeHeader(req.hdrs, SIP_HDR_USER_AGENT);
  } else if (AmConfig::Signature.length() &&
             getHeader(req.hdrs, SIP_HDR_USER_AGENT).empty()) {
    // Inject signature when no User-Agent is already present. This covers both
    // SEMS-originated requests and SBC-relayed requests where the upstream UAC
    // sent no User-Agent (e.g. auth-retry REGISTERs generated by UACAuth).
