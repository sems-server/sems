...
static string make_request(const char *method, const char *ruri, const char *cseq_method,
...
static string make_req(const char *method, const char *ruri, const char *extra_hdrs = "",
                       const char *body = NULL) {
  return make_request(method, ruri, method, extra_hdrs, body);
}

static string make_response(int code, const char *reason, const char *cseq_method = "INVITE",
...
FCTMF_SUITE_BGN(test_rfc3261_musts) {
  ...
  FCT_TEST_BGN(rfc3261_20_8_callid_extracted) {
    ...
    fct_chk(memcmp(msg.callid->value.s, "a84b4c76e66710@pc33.atlanta.com", msg.callid->value.len) == 0);
  }
  FCT_TEST_END();
  ...
  FCT_TEST_BGN(rfc3261_20_42_via_tcp_transport) {
    const char *raw = "INVITE sip:bob@biloxi.com SIP/2.0\r\n"
  ...
  FCT_TEST_BGN(rfc3261_8_1_1_7_branch_without_magic_cookie) {
    ...
    fct_chk(msg.via_p1->branch.len < MAGIC_BRANCH_LEN ||
            memcmp(msg.via_p1->branch.s, MAGIC_BRANCH_COOKIE, MAGIC_BRANCH_LEN) != 0);
  }
  FCT_TEST_END();
  ...
  FCT_TEST_BGN(rfc3261_7_4_body_with_content_length) {
    ...
    string raw = make_req("INVITE", "sip:bob@biloxi.com", "Content-Type: application/sdp\r\n", sdp);
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc == 0);
  ...
  FCT_TEST_BGN(rfc3261_7_3_1_unknown_headers_preserved) {
    ...
    int other_count = 0;
    for (list<sip_header *>::iterator it = msg.hdrs.begin(); it != msg.hdrs.end(); ++it) {
  ...
  FCT_TEST_BGN(rfc3261_8_1_1_5_cseq_options_vs_invite) {
    ...
    string raw = make_request("OPTIONS", "sip:bob@biloxi.com", "INVITE", "");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc != 0);
  ...
  FCT_TEST_BGN(rfc3261_20_30_record_route_in_response) {
    string raw = make_response(200, "OK", "INVITE", "Record-Route: <sip:proxy.atlanta.com;lr>\r\n");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc == 0);
  ...
  FCT_TEST_BGN(rfc3261_20_22_max_forwards_recognized) {
    string raw = make_req("INVITE", "sip:bob@biloxi.com", "Max-Forwards: 70\r\n");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc == 0);
  ...
  FCT_TEST_BGN(rfc3261_20_32_require_recognized) {
    string raw = make_req("INVITE", "sip:bob@biloxi.com", "Require: 100rel\r\n");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc == 0);
  ...
  FCT_TEST_BGN(rfc3261_20_10_multiple_contacts) {
    const char *raw = "REGISTER sip:registrar.biloxi.com SIP/2.0\r\n"
                      "Via: SIP/2.0/UDP 192.0.2.1;branch=z9hG4bK776\r\n"
                      "To: <sip:bob@biloxi.com>\r\n"
  ...
  FCT_TEST_BGN(rfc3261_20_16_cseq_large_number) {
    const char *raw = "BYE sip:bob@biloxi.com SIP/2.0\r\n"
                      "Via: SIP/2.0/UDP 192.0.2.1;branch=z9hG4bK776\r\n"
                      "To: <sip:bob@biloxi.com>\r\n"
                      "From: <sip:alice@atlanta.com>;tag=xyz\r\n"
                      "Call-ID: abc@host\r\n"
  ...
  FCT_TEST_BGN(rfc3261_19_1_ruri_phone_context) {
    ...
    string raw = make_req("INVITE", "sip:+12125551234@gateway.example.com;user=phone");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk_eq_int(rc, 0);
  ...
  FCT_TEST_BGN(rfc3261_19_1_ruri_multiple_params) {
    string raw = make_req("INVITE", "sip:bob@biloxi.com;transport=udp;user=phone;lr");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk_eq_int(rc, 0);
  ...
  FCT_TEST_BGN(rfc3261_19_1_ruri_with_headers) {
    string raw = make_req("INVITE", "sip:bob@biloxi.com?Subject=Meeting&Priority=urgent");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk_eq_int(rc, 0);
  ...
  FCT_TEST_BGN(rfc3261_8_1_1_5_bye_cseq_cancel_mismatch) {
    string raw = make_request("BYE", "sip:bob@biloxi.com", "CANCEL", "");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc != 0);
  ...
  FCT_TEST_BGN(rfc3261_8_1_1_5_register_cseq_bye_mismatch) {
    string raw = make_request("REGISTER", "sip:registrar.biloxi.com", "BYE", "");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc != 0);
