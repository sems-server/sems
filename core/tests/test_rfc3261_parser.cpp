...
static string make_request(const char *method, const char *ruri, const char *extra_hdrs,
...
static string make_response(int code, const char *reason, const char *extra_hdrs = NULL) {
  char status_line[64];
...
FCTMF_SUITE_BGN(test_rfc3261_parser) {
  ...
  FCT_TEST_BGN(rfc3261_8_2_2_1_bad_version_request) {
    ...
    const char *raw = "INVITE sip:bob@biloxi.com SIP/3.0\r\n"
                      "Via: SIP/2.0/UDP 192.0.2.1;branch=z9hG4bK776\r\n"
                      "To: <sip:bob@biloxi.com>\r\n"
                      "From: <sip:alice@atlanta.com>;tag=xyz\r\n"
                      "Call-ID: abc@host\r\n"
  ...
  FCT_TEST_BGN(rfc3261_8_2_2_1_bad_version_minor) {
    ...
    const char *raw = "OPTIONS sip:bob@biloxi.com SIP/2.1\r\n"
                      "Via: SIP/2.0/UDP 192.0.2.1;branch=z9hG4bK776\r\n"
                      "To: <sip:bob@biloxi.com>\r\n"
                      "From: <sip:alice@atlanta.com>;tag=xyz\r\n"
                      "Call-ID: abc@host\r\n"
  ...
  FCT_TEST_BGN(rfc3261_via_branch_extracted) {
    ...
    fct_chk(memcmp(msg.via_p1->branch.s, MAGIC_BRANCH_COOKIE, MAGIC_BRANCH_LEN) == 0);
  }
  FCT_TEST_END();
  ...
  FCT_TEST_BGN(rfc3261_contact_collected) {
    string raw = make_request("INVITE", "sip:bob@biloxi.com", "Contact: <sip:alice@pc33.atlanta.com>\r\n");
    sip_msg msg;
    char *err_msg = NULL;
    int rc = try_parse(raw.c_str(), raw.size(), msg, err_msg);
    fct_chk(rc == 0);
  ...
  FCT_TEST_BGN(rfc3261_compact_header_forms) {
    ...
    const char *raw = "OPTIONS sip:bob@biloxi.com SIP/2.0\r\n"
  ...
  FCT_TEST_BGN(rfc3261_method_recognition) {
    ...
    struct {
      ...
      int expected;
        ...
        {"INVITE", sip_request::INVITE}, {"ACK", sip_request::ACK},       {"OPTIONS", sip_request::OPTIONS},
        {"BYE", sip_request::BYE},       {"CANCEL", sip_request::CANCEL}, {"REGISTER", sip_request::REGISTER},
    };
    for (int i = 0; i < 6; i++) {
  ...
  FCT_TEST_BGN(rfc3261_response_codes_parsed) {
    ...
    const char *reasons[] = {"Trying", "Ringing", "OK", "Moved", "Bad Request", "Server Error", "Decline"};
    for (int i = 0; i < 7; i++) {
