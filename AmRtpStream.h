/*
 * $Id: AmRtpStream.h,v 1.25.2.6 2005/06/01 12:00:24 rco Exp $
 *
...
class AmAudio;
class AmSession;
class AmRtpStreamSet;
// class AmRtpScheduler;
class SdpPayload;
...
public:
    ...
    /**
     * Turn packet tracing on or off.
     */
    void trace(bool on, const string& id = "");

    friend class AmRtpStreamSet;
};
...
{
public:
    typedef map<AmRtpStream*,AmRtpStreamInfo> StreamSet;

    AmRtpStreamSet();
    ~AmRtpStreamSet();

    void addStream(AmRtpStream* str, const AmRtpStreamInfo& info);
    void removeStream(AmRtpStream* str);
    void setStrInfo(AmRtpStream* str, const AmRtpStreamInfo& info);

    // send & receive
    int exchange();

    unsigned int getTS() { return ts_set; }

    void enableRTPTrace(const string& trace_file);

private:
//     AmRtpScheduler* sched;
    
    unsigned int    ts_set;
    int             nb_packet;
    StreamSet       stream_set;

    struct timeval ts_clock_begin;

    auto_ptr<AmRtpPacketTracer> tracer;
};
