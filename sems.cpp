...
static void sig_usr_fifo(int signo)
{
    WARN("signal %d received\n", signo);
    
    if((signo != SIGCHLD) && is_main){
...
static int use_args(char* progname, map<char,string>& args)
{
	 ...
	 case 'o':
	     AmConfig::SerFIFO = it->second;
	     break;
	     
	 case 'i':
	     AmConfig::FifoName = it->second;
 	    break;
...
int main(int argc, char* argv[])
{
    ...
    printf( "\n\nConfiguration:\n"
	    ...
	    AmConfig::SerFIFO.c_str(),
	    AmConfig::FifoName.c_str(),
    ...
    sighandler_t sig_usr=0;
    if(!AmConfig::FifoName.empty())
	sig_usr = sig_usr_fifo;
    else
	sig_usr = sig_usr_un;

    if(set_sighandler(sig_usr_fifo))
	return -1;

    if(AmConfig::init())
	return -1;

    AmFifoServer* fifo_srv=0;
    if(!AmConfig::FifoName.empty()){
	fifo_srv = AmFifoServer::instance();
	if(fifo_srv->init(AmConfig::FifoName.c_str()))
	    fifo_srv = 0;
    }

    DBG("Loading application plug-ins\n");
    if(AmPlugIn::instance()->load(AmConfig::PlugInPath + "/apps", AmPlugIn::App))
	return -1;

    DBG("Loading audio plug-ins\n");
    if(AmPlugIn::instance()->load(AmConfig::PlugInPath + "/audio", AmPlugIn::Audio))
	return -1;
...
//     DBG("Starting RTP scheduler\n");
//     AmRtpScheduler::instance()->start();
	...
	un_srv = AmUnServer::instance();
	if(un_srv->init(AmConfig::SocketName.c_str()))
	    un_srv = 0;
    }
...
static string getLocalIP(const string& dev_name)
{
    ...
    if(sd == -1){
	ERROR("setLocalIP: socket: %s",strerror(errno));
	exit(-1);
    }	
	...
	if(ioctl(sd,SIOCGIFADDR,&ifr)!=0){
	    ERROR("setLocalIP: ioctl: %s",strerror(errno));
	    exit(-1);
	}
