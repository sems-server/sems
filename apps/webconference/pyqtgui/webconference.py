...
print(
    "Server Control URI: '%s' - change CONTROL_URI to use local SEMS instance"
    % CONTROL_URI
)
...
class named_participant(Ui_participant):
    def __init__(self, parent, title, callid, id):
        ...
        QtCore.QObject.connect(
            self.bt_ciao, QtCore.SIGNAL("clicked()"), self.ciao_clicked
        )
        QtCore.QObject.connect(
            self.cb_muted, QtCore.SIGNAL("stateChanged(int)"), self.muted_changed
        )
    ...
    def muted_changed(self, i):
        ...
        self.cb_muted.emit(
            QtCore.SIGNAL("mute(int,bool)"), self.id, self.cb_muted.isChecked()
        )
...
class StartQT4(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        if HAS_ACCOUNT:
            ...
            if dlg.exec_() == QDialog.Rejected:
                raise Exception("well, I need a SIP account to make calls")
        ...
        QtCore.QObject.connect(
            self.ui.buttonNew, QtCore.SIGNAL("clicked()"), self.new_call
        )
        ...
        for i in range(10):
            ...
            code, result, adminpin, serverstatus = self.s.di(
                "webconference", "roomCreate", self.roomname
            )
            ...
            if code == 0:
                ...
                break
        if self.adminpin == "":
            raise Exception("oh, could not get a free room :(")
    ...
    def createparticipantWidget(self, name, callid):
        ...
        QtCore.QObject.connect(
            part.cb_muted, QtCore.SIGNAL("mute(int,bool)"), self.part_muted
        )
        self.addDockWidget(
            random.choice(
                [
