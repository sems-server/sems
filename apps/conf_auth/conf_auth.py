...
server_path = (
    XMLRPC_PROTOCOL + "://" + XMLRPC_SERVER + ":" + str(XMLRPC_PORT) + XMLRPC_DIR
)
...
class IvrDialog(IvrDialogBase):
    ...
    def onSessionStart(self):
        self.sessionInfo()
        self.setNoRelayonly()
        self.welcome_msg = IvrAudioFile.tts(
            "Welcome to the conferencing server. "
            + "Please enter your PIN number, followed by the star key."
        )
        self.auth_ok_msg = IvrAudioFile.tts(
            "Your PIN number is correct, you will be connected now."
        )
        # 		self.auth_fail_msg = IvrAudioFile.tts("Sorry, you have entered an invalid PIN number. " + \
        # 							"Please try again.")
        self.hint_msg = IvrAudioFile.tts(
            "Please enter your PIN number, followed by the star key."
        )
    ...
    def onDtmf(self, key, duration):
        if self.state == collect:
            ...
            if key == 10:
                ...
                erg = c.AuthorizeConference(
                    self.dialog.remote_uri, self.dialog.local_party, self.keys
                )
    ...
    def onTimer(self, id):
        if id == HINT_TIMER and self.state == collect:
            ...
            debug(
                "conference timer timeout. disconnecting "
                + self.conf_to
                + " uri: "
                + self.conf_uri
            )
