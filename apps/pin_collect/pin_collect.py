...
if config.get("auth_mode", None) == "XMLRPC":
    import xmlrpc
...
class IvrDialog(IvrDialogBase):
    ...
    def onDtmf(self, key, duration):
        ...
        if self.state == collect:
            ...
            if key < 10:
            ...
            elif key == 10: # star key (*) was pressed
                ...
                if config.get("auth_mode", None) == "XMLRPC":
                ...
                elif config.get("auth_mode", None) == "REFER":
                    self.state = connect
                    self.removeTimer(HINT_TIMER)
                    self.transfer_cseq = self.dialog.cseq
                    self.refer(
                    ...
                    debug("saved remote_uri: " + self.dlg_remote_uri)
                    self.transfer_cseq = self.dialog.cseq
                    # transfer to ivr_test module
                    sip_uri = "sip:ivr_test@{}:5060".format(self.dialog.domain)
                    # sip_uri = f"sip:{self.dialog.user}+{self.keys}@{self.dialog.domain}"
                    debug("Redirecting to: " + sip_uri)
                    #self.redirect(sip_uri)
                    self.refer(sip_uri)
                    self.bye()
                    self.stopSession()
            elif key == 11: # hash key (#) was pressed
                debug("TEST")
                self.state = connect
                self.removeTimer(HINT_TIMER)
                self.dlg_remote_uri = self.dialog.remote_uri
                ...
                self.transfer_cseq = self.dialog.cseq
                # transfer to ivr_test module
                sip_uri = "sip:ivr_test2@{}:5060".format(self.dialog.domain)
                # sip_uri = f"sip:{self.dialog.user}+{self.keys}@{self.dialog.domain}"
                debug("Redirecting to: " + sip_uri)
                #self.redirect(sip_uri)
                self.refer(sip_uri)
    ...
    def onSipReply(self, reply, old_status):
        debug("onSipReply() fired")
        debug("  reply.cseq: {}".format(reply.cseq))
