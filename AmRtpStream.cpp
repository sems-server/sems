...
bool ts_less::operator()(const unsigned int& l, 
...
void AmRtpStream::setLocalPort()
{
	...
	port = (rand() % 
		(AmConfig::RtpHighPort 
		 - AmConfig::RtpLowPort + 1)) 
	    +  AmConfig::RtpLowPort;

	// only even port number
	port &= 0xfffe;
    ...
    if (!retry){
	ERROR("could not find a random local port !\n");
	close(sd);
	throw string("while setting random local port");
    }
			  ...
			  unsigned int& ts, unsigned int audio_buffer_ts)
{
    ...
    if(tracer)
	tracer->write_packet(audio_buffer_ts,&recv_addr,&rp);
...
void AmRtpStream::bufferPacket(AmRtpPacket* p)
{
    jitter_mut.lock();
    gettimeofday(&last_recv_time,NULL);
    jitter_buf.insert(std::make_pair(p->timestamp,p));
    jitter_mut.unlock();
}
...
void AmRtpStream::trace(bool on, const string& id)
{
    if((tracer && on)
	|| (!tracer && !on))
	return;
    ...
    tracer_mut.lock();
    if(!tracer && on){
	tracer = new AmRtpPacketTracer();
	if(tracer->open("/tmp/" + id + ".trace")==-1){
	    delete tracer;
	    tracer = 0;
	}
    }
