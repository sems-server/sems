/*
 * $Id: ilbc.c,v 1.4.2.2 2005/08/25 06:55:13 rco Exp $
 *
...
BEGIN_EXPORTS( "ilbc" )
    ...
    BEGIN_CODECS
      CODEC( CODEC_ILBC,  1, Pcm16_2_iLBC, iLBC_2_Pcm16, (amci_codec_init_t)iLBC_create, (amci_codec_destroy_t)iLBC_destroy )
    END_CODECS
...
long iLBC_create(const char* format_parameters, amci_codec_fmt_info_t* format_description) {
  ...
  initDecode(&codec_inst->iLBC_Dec_Inst, mode, 1 /* use_enhancer */);
			 ...
			 unsigned int channels, unsigned int rate, long h_codec )
{
  ...
  for (i=0;i< size / codec_inst->iLBC_Dec_Inst.no_of_bytes;i++) {
    ...
    iLBC_decode(decblock, (unsigned char *)(in_buf + i*codec_inst->iLBC_Dec_Inst.no_of_bytes),
		&codec_inst->iLBC_Dec_Inst, 1 /* mode 0=PL, 1=Normal */  );
       /* convert to short */
