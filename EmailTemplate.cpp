/*
 * $Id: EmailTemplate.cpp,v 1.3.2.2 2005/08/25 06:55:12 rco Exp $
 *
...
string EmailTemplate::replaceVars(const string& str,const AmCmd& cmd) const
{
...
AmMail EmailTemplate::getEmail(const AmCmd& cmd) const
{
    try {
	return AmMail(replaceVars(from,cmd),
		      replaceVars(subject,cmd),
		      replaceVars(to,cmd),
		      replaceVars(body,cmd));
    }
